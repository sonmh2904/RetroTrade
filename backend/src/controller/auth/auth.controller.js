...
// Helper function để fetch (tương tự như trong verify.controller)
...
// Xử lý Facebook OAuth callback với authorization code
module.exports.handleFacebookCallback = async (req, res) => {
    try {
        ...
        const profileUrl = `https://graph.facebook.com/me?fields=id,name,email,picture.width(256).height(256)&access_token=${accessToken}`;
        ...
        const frontendUrl = process.env.FRONTEND_URL || "http://localhost:3000";
        return res.redirect(`${frontendUrl}/auth/login?error=${encodeURIComponent(error.message)}`);
    }
};
