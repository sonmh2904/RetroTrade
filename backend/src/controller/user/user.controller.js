...
module.exports.getAllUsers = async (req, res) => {
  try {
    ...
    if (
      role &&
      role !== "all" &&
      ["renter", "owner", "admin", "moderator"].includes(role)
    ) {
      filter.role = role;
    }
    ...
    if (search && search.trim()) {
      ...
      if (filter.$or && Array.isArray(filter.$or)) {
        ...
        delete filter.$or;
        filter.$and = [{ $or: statusOr }, searchConditions];
      } else {
    ...
    const query = User.find(filter).select(
      "userGuid email fullName displayName avatarUrl role isEmailConfirmed isPhoneConfirmed isIdVerified reputationScore points isDeleted isActive createdAt"
    );
...
module.exports.getUsersForModeration = async (req, res) => {
  try {
    ...
    disputes.forEach((d) => {
      if (d.reportedUserId)
        userIdsWithDisputes.add(d.reportedUserId.toString());
    });
    ...
    const filter = {
      _id: {
        $in: Array.from(userIdsWithIssues).map(
    ...
    return res.json({
      code: 500,
      message: "Lấy danh sách người dùng cần xử lý thất bại",
      error: error.message,
    });
  }
};
...
module.exports.getUserById = async (req, res) => {
  try {
    const user = await User.findById(req.params.id).select(
      "userGuid email fullName displayName avatarUrl bio role isEmailConfirmed isPhoneConfirmed isIdVerified reputationScore points createdAt updatedAt"
    );
    if (!user)
...
module.exports.getProfile = async (req, res) => {
  try {
    ...
    const user = await User.findOne({
      ...
      .select(
        "userGuid email fullName displayName avatarUrl bio phone isEmailConfirmed isPhoneConfirmed isIdVerified reputationScore points role lastLoginAt createdAt updatedAt"
      )
      .lean();
...
module.exports.banUser = async (req, res) => {
  try {
    ...
    if (currentUserRole === "moderator") {
      ...
      if (dispute) {
        hasValidReason = true;
        validationDetails.push(`Có Khiếu nại(ID: ${dispute._id})`);
      }
      ...
      if (!hasValidReason) {
        return res.json({
          code: 403,
          message:
            "Bạn chỉ có thể khóa tài khoản khi người dùng có khiếu nại, bị báo cáo hoặc có Khiếu nại. Vui lòng kiểm tra lại thông tin người dùng.",
        });
      }
    }
    ...
    try {
      await AuditLog.create({
        ...
        ChangedAt: new Date(),
        ChangeSummary: `Khóa tài khoản người dùng: ${
          user.fullName || user.email || user.phone || user._id
        }. Lý do: ${reason.trim()}`,
      });
    } catch (auditError) {
    ...
    try {
                            ...
                            <div style="background-color: #dbeafe; border-left: 4px solid #3b82f6; padding: 16px; margin: 20px 0;">
                                ...
                                <p style="color: #1e40af; font-size: 14px; margin: 4px 0;">
                                    <strong>Người thực hiện:</strong> ${
                                      admin.fullName || admin.email || admin._id
                                    }
                                </p>
                                <p style="color: #1e40af; font-size: 14px; margin: 4px 0;">
      ...
      console.error(
        "Error sending ban notification email to system:",
        systemEmailError
      );
...
module.exports.unbanUser = async (req, res) => {
  try {
    ...
    try {
      ...
      console.log(
        `Updated ${updateResult.modifiedCount} ban history records for user ${user._id}`
      );
    ...
    try {
      await AuditLog.create({
        ...
        ChangedByUserId: adminId,
        ChangedAt: new Date(),
        ChangeSummary: `Mở khóa tài khoản người dùng: ${
          user.fullName || user.email || user.phone || user._id
        }`,
      });
    } catch (auditError) {
